...
def checkUpdateTimes(items):
    ...
    for item in items:
        try:
            mc.execute("select utime from {0} order by utime desc limit 1;".format(item))
...
processList = ["save5n1.py", "giveweather.py", "savebarometer.py"]
recordList = ["barometer", "ftemperature", "rtemperature", "humidity", 
                "raincounter"]
...
if __name__ == '__main__':
    ...
    ipAddress=hv["weatherHealth"]["ipAddress"]
    port = hv["weatherHealth"]["port"]
